...
dkms-install:
	@echo "Installing $(MODNAME) via DKMS..."
	...
	sudo usermod -aG linuwu_sense $(REAL_USER)
	@echo "Setting permissions via tmpfiles..."
	@model_path=$$(ls /sys/module/$(MODNAME)/drivers/platform:acer-wmi/acer-wmi/ 2>/dev/null | grep -E 'predator_sense|nitro_sense' || true); \
	if [ -n "$$model_path" ]; then \
		echo "Detected model directory: $$model_path"; \
		conf_file="/etc/tmpfiles.d/$(MODNAME).conf"; \
		[ -f $$conf_file ] || sudo touch $$conf_file; \
	...
	@echo "Module $(MODNAME) installed via DKMS."

dkms-uninstall:
	@echo "Uninstalling $(MODNAME) from DKMS..."
	@sudo dkms remove -m $(MODNAME) -v 1.0 --all || true
	@sudo rm -rf /usr/src/$(MODNAME)-1.0
	@sudo rm -f /etc/modules-load.d/$(MODNAME).conf
	@sudo rm -f /etc/modprobe.d/blacklist-acer_wmi.conf
	@sudo systemctl stop linuwu_sense.service || true
	@sudo systemctl disable linuwu_sense.service || true
	@sudo rm -f /etc/systemd/system/linuwu_sense.service
	@sudo systemctl daemon-reload
	@sudo rmmod $(MODNAME) 2>/dev/null || true
	@sudo modprobe acer_wmi
	...
	@sudo rm -f /etc/tmpfiles.d/$(MODNAME).conf
	@echo "Uninstalled $(MODNAME) (DKMS) and cleaned up."
